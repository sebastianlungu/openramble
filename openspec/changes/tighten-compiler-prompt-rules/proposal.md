...
- **Reject capture-chrome in the visible prompt.** The existing forbidden-phrase list gains `recording pill`, `capture banner`, `capture pill`, `floating banner`. The downstream coding agent can no longer be told to "rebuild" OpenVysta's own capture overlay.
...
- **Out of scope (deferred to a follow-up change)**: project-context discovery (`--context auto|file:<path>`), post-enrichment scoring against the AGENTS.md scorecard, the Swift `SCContentFilter` exclusion of the OpenVysta capture banner in `ScreenCapture.swift`. None of these are required for this change to ship.
