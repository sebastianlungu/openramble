...
The public CLI contract SHALL NOT include a browser metadata input. The `openvysta compile` command MUST fail loudly when invoked with legacy browser metadata flags rather than accepting, ignoring, staging, or redacting the file.
...
- **WHEN** the user invokes `openvysta compile --browser ./browser.json` with otherwise valid inputs
...
- **AND** no build, test, README, PRD, or package instruction requires installing a Chrome extension for OpenVysta
...
- **AND** the PRD explicitly preserves the thesis that OpenVysta is not a DOM operator or browser automation product
