...
final class ScreenCapture: NSObject, SCStreamDelegate, SCStreamOutput {
    ...
    private let bufferQueue = DispatchQueue(label: "ai.omnicaptain.buffer")
    ...
    nonisolated func startCapture(
        ...
        try stream?.addStreamOutput(self, type: .screen, sampleHandlerQueue: DispatchQueue(label: "ai.omnicaptain.screen"))
    ...
    private func prepareLocalRecording(width: Int, height: Int, runDirectory: URL?) {
        ...
        if let runDirectory {
            ...
            dir = home.appendingPathComponent(".omnicaptain/runs/omni_\(iso)/")
